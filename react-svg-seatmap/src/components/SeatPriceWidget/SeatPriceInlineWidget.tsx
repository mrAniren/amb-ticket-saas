...
import './SeatPriceInlineWidget.scss';

interface SeatPriceInlineWidgetProps {
...
export const SeatPriceInlineWidget: React.FC<SeatPriceInlineWidgetProps> = ({
  selectedSeats,
  prices,
  onApplyPrice,
  onRemovePrice
  ...
  return (
    <div className="seat-price-inline-widget">
      <div className="seat-price-inline-widget__header">
        <h4>Установить цену для {selectedSeats.length} мест:</h4>
      </div>

      <div className="seat-price-inline-widget__content">
        {prices.length === 0 ? (
          <div className="seat-price-inline-widget__empty">
            <p>Нет доступных цен. Добавьте цены ниже.</p>
          </div>
        ) : (
          <div className="price-options-inline">
            {prices.map(price => (
              <button
                key={price.id}
                onClick={() => onApplyPrice(price.id)}
                className="price-option-inline"
                title={`Применить цену ${price.value} ${price.currency}`}
              >
                <div className="price-option-inline__color" style={{ backgroundColor: getPriceColor(price.value) }} />
                <span className="price-option-inline__value">{price.value} {price.currency}</span>
              </button>
            ))}
            
            <button
              onClick={onRemovePrice}
              className="price-option-inline price-option-inline--remove"
              title="Убрать цену"
            >
              <div className="price-option-inline__color price-option-inline__color--remove">×</div>
              <span className="price-option-inline__value">Убрать</span>
            </button>
          </div>
        )}
      </div>
