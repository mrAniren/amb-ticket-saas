...
export const PriceCanvasWithSelection: React.FC<PriceCanvasProps> = ({
  ...
  // Функция для проверки, активно ли место
  const isSeatActive = useCallback((): boolean => {
    return true; // Все места всегда активны
  }, []);
  ...
  const renderCanvas = useCallback(() => {
    ...
    sortedSeats.forEach((seat) => {
      ...
      const isActive = isSeatActive(); // Проверяем активность места
  ...
  const handleMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isSelecting && interactionMode === InteractionMode.SELECTION) {
      ...
      const hoveredSeatElement = canvasSeats.find(seat =>
        ...
        isSeatActive() // Только активные места могут быть подсвечены
      );
  ...
  useEffect(() => {
    renderCanvas();
  }, [canvasSeats, selectedSeats, hoveredSeat, transform, canvasSize, isSelecting, selectionStart, selectionEnd, interactionMode]);
